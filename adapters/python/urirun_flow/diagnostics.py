...
PLAYBOOK: list[_Rule] = [
    ...
    _Rule(
        "llm-provider-quota",
        [r"key limit exceeded", r"quota", r"rate limit", r"resource exhausted",
         r"insufficient credits", r"billing.*limit"],
        "The LLM provider rejected planning because an account, key, billing, or rate quota was exhausted.",
        lambda t: [
            {"id": "switch-model-or-key", "kind": "planner", "automatic": False,
             "label": "Switch to a key/model with available quota or retry after the provider limit resets."},
        ],
        confidence=0.9,
    ),
    _Rule(
