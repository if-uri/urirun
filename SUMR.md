...
# code2llm | 33f 9748L | python:18,yaml:3,javascript:3,shell:2,json:2,c:1,cpp:1,toml:1 | 2026-06-19
# generated in 0.01s
# CC̅=4.7 | critical:22/453 | dups:0 | cycles:0
...
PIPELINES[110]:
  ...
  [9] Src [copy_token]: copy_token → memcpy → is_path_end
      PURITY: 100% pure
  [10] Src [call]: call → _post
      PURITY: 100% pure
  [11] Src [run_spawn]: run_spawn → render_command → render_value
      PURITY: 100% pure
  [12] Src [run_shell_template]: run_shell_template → render_value
      PURITY: 100% pure
  [13] Src [run_docker_exec]: run_docker_exec → render_command → render_value
      PURITY: 100% pure
  [14] Src [run_docker_run]: run_docker_run → render_command → render_value
      PURITY: 100% pure
  [15] Src [run_fetch]: run_fetch → render_value
      PURITY: 100% pure
  [16] Src [run_local_function]: run_local_function
      PURITY: 100% pure
  [17] Src [run_mqtt_publish]: run_mqtt_publish
      PURITY: 100% pure
  [18] Src [main]: main → load_registry_arg → compile_registry → expand_bindings → ...(1 more)
      PURITY: 100% pure
  [19] Src [dispatch]: dispatch → parse_uri
      PURITY: 100% pure
  [20] Src [run_spawn]: run_spawn → _truncate
      PURITY: 100% pure
  [21] Src [run_shell_template]: run_shell_template → _truncate
      PURITY: 100% pure
  [22] Src [run_fetch]: run_fetch → _truncate
      PURITY: 100% pure
  [23] Src [run_local_function]: run_local_function → fn
      PURITY: 100% pure
  [24] Src [run_mqtt_publish]: run_mqtt_publish
      PURITY: 100% pure
  [25] Src [main]: main → load_registry_arg
      PURITY: 100% pure
  [26] Src [_dumps]: _dumps
      PURITY: 100% pure
  [27] Src [_loads]: _loads
      PURITY: 100% pure
  [28] Src [stream]: stream → channel_target
      PURITY: 100% pure
  [29] Src [list_routes]: list_routes → channel_target
      PURITY: 100% pure
  [30] Src [main]: main → serve → _route_list
      PURITY: 100% pure
  [31] Src [main]: main → merge_into
      PURITY: 100% pure
  [32] Src [call_tool]: call_tool → build_tool_index → to_mcp_tools → tool_name
      PURITY: 100% pure
  [33] Src [main]: main → serve_mcp → to_mcp_tools → tool_name
      PURITY: 100% pure
  [34] Src [uri_handler]: uri_handler → normalize_route_entry → default_adapter
      PURITY: 100% pure
  [35] Src [discover_entry_points]: discover_entry_points → route_from_uri → parse_uri
      PURITY: 100% pure
  [36] Src [hydrate_registry]: hydrate_registry → _walk_route_entries
      PURITY: 100% pure
  [37] Src [exec_local_function]: exec_local_function → fn
      PURITY: 100% pure
  [38] Src [exec_fetch]: exec_fetch
      PURITY: 100% pure
  [39] Src [exec_spawn]: exec_spawn
      PURITY: 100% pure
  [40] Src [exec_shell_template]: exec_shell_template
      PURITY: 100% pure
  [41] Src [exec_mqtt_publish]: exec_mqtt_publish
      PURITY: 100% pure
  [42] Src [main]: main → _emit_json → write_json
      PURITY: 100% pure
  [43] Src [main]: main → scan_path → iter_project_files
      PURITY: 100% pure
  [44] Src [create_llm_session]: create_llm_session → init_db → connection → connect → ...(1 more)
      PURITY: 100% pure
  [45] Src [add_llm_message]: add_llm_message → init_db → connection → connect → ...(1 more)
      PURITY: 100% pure
  [46] Src [run_uri_route]: run_uri_route → route_db_path
      PURITY: 100% pure
  [47] Src [run_uri_route]: run_uri_route → _domain
      PURITY: 100% pure
  [48] Src [uri_shell]: uri_shell → uri_command → model_from_function
      PURITY: 100% pure
  [49] Src [decorated_bindings]: decorated_bindings
      PURITY: 100% pure
  [50] Src [render_sequence]: render_sequence → render_value
      PURITY: 100% pure

LAYERS:
  adapters/                       CC̄=4.9    ←in:4  →out:0
  │ !! v2                        1618L  0C   60m  CC=31     ←0
...
COUPLING:
                   adapters.python            v1.js         adapters
...
# code2llm/evolution | 453 func | 24f | 2026-06-19
# generated in 0.00s
...
NEXT[10] (ranked by impact):
  ...
  [10] !  SPLIT-FUNC      host_command  CC=19  fan=18
      WHY: CC=19 exceeds 15
      EFFORT: ~1h  IMPACT: 342


RISKS[3]:
  ⚠ Splitting adapters/python/urirun/v2.py may break 60 import paths
